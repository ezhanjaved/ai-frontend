...
export default class Strategy extends Phaser.Scene {
  ...
  create() {
    ...
    this.timer.setCallback((timeLeft) => {
      ...
      if (timeLeft <= 0) {
        ...
        this.accurateMoves = (this.perfect_run/this.aiMoves)*100;
        console.log("Wizard Accurate Hit %: " + this.accurateMoves);
        this.registry.set('wizard-accurate', this.accurateMoves); 
  ...
  startPlayerTurn() {
    this.turn = "player";
    const perfect_moves = [23, 21, 16, 14, 9, 7, 2]
    if (perfect_moves.includes(this.boxes.children.size)) {
      this.perfect_run += 1;
    }
    this.updateButtons();
  ...
  update() {
    ...
    console.log("Boxes Left: " + this.boxes.children.size)
    console.log("Turn: " + this.turn)
    console.log("AI MOVES: " + this.aiMoves)
    console.log("Perfect Moves: " + this.perfect_run)
  }
}