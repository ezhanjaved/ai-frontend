...
export default class devilScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.attackKeys = this.input.keyboard.addKeys({
          One: Phaser.Input.Keyboard.KeyCodes.ONE, // Key for "1"
          Two: Phaser.Input.Keyboard.KeyCodes.TWO, // Key for "2"
          Three: Phaser.Input.Keyboard.KeyCodes.THREE, // Key for "3"
          Four: Phaser.Input.Keyboard.KeyCodes.FOUR, // Key for "4"
    });
    ...
    this.timer.setCallback((timeLeft) => {
      ...
      if (timeLeft <= 0) {
        this.devilAccurate = (this.successfulAttackHits/this.attackHits)*100;
        console.log("Devil Accurate Hit %: " + this.devilAccurate);
        this.registry.set('devil-accurate', this.devilAccurate); 
  ...
  update() {
    ...
    async function getEnemyAction(gameState) {
      try {
        const response = await fetch("https://ai-backend-xkbh.onrender.com/predict_advance_action", {
          method: "POST",
          headers: {
